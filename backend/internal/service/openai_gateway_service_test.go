...
func TestOpenAICompactNonStreamingJSONCanReturnClientSSE(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	resp := &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_cmp_1","object":"response","model":"gpt-5.5","status":"completed","output":[{"id":"cmp_1","type":"compaction","status":"completed","encrypted_content":"payload"}],"usage":{"input_tokens":2,"output_tokens":1,"total_tokens":3}}`)),
	}

	svc := &OpenAIGatewayService{}
	result, err := svc.handleNonStreamingResponse(context.Background(), resp, c, &Account{ID: 1}, "gpt-5.5", "gpt-5.5")
	require.NoError(t, err)
	require.NotNil(t, result)
	...
	require.Equal(t, "cmp_1", gjson.Get(events[0][1], "item.id").String())
	require.Equal(t, "response.completed", events[1][0])
	require.Equal(t, "resp_cmp_1", gjson.Get(events[1][1], "response.id").String())
	require.Equal(t, int64(1), gjson.Get(events[1][1], "response.output.#").Int())
}
