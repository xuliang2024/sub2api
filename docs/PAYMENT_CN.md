...
| **商户证书序列号** | 商户证书序列号 | 是 |
| **微信支付公钥** | 微信支付公钥（PEM 格式）；留空则使用平台证书自动下载模式 | 否 |
| **微信支付公钥 ID** | 微信支付公钥 ID；与微信支付公钥成对填写 | 否 |

> 微信官方支付支持两种验签模式：
> 1. **微信支付公钥模式**：填写 `微信支付公钥` 与 `微信支付公钥 ID`，适合新商户号。
> 2. **平台证书自动下载模式**：不填写上述两个字段，仅使用 AppID、商户号、商户私钥、商户证书序列号、APIv3 密钥，系统会通过微信支付 API 下载并定时刷新平台证书。旧系统若只保存了商户私钥、商户证书序列号和 APIv3 密钥，可使用此模式。两个公钥字段必须同时填写或同时留空。
